...
const schema = z.object({ name: z.string().min(2), phone: z.string().optional(), email: z.union([z.literal(''), z.email()]).optional(), address: z.string().optional(), notes: z.string().optional(), stock_status: z.enum(['selling', 'returned', 'sold_out']), is_active: z.enum(['true', 'false']) })
...
export function ConsignorsPage() {
  ...
  const form = useForm<Form>({ resolver: zodResolver(schema), defaultValues: { name: '', phone: '', email: '', address: '', notes: '', stock_status: 'selling', is_active: 'true' } })
  ...
  const save = useMutation({ mutationFn: (values: Form) => { const payload = { ...values, is_active: values.is_active === 'true' }; return editing ? api.put(`/consignors/${editing.id}`, payload) : api.post('/consignors', payload) }, onSuccess: () => { qc.invalidateQueries({ queryKey: ['consignors'] }); close() } })
  ...
  const edit = (item: Consignor) => { setEditing(item); form.reset({ name: item.name, phone: item.phone ?? '', email: item.email ?? '', address: item.address ?? '', notes: item.notes ?? '', stock_status: item.stock_status ?? 'selling', is_active: item.is_active === false ? 'false' : 'true' }); setOpen(true) }
  ...
  const stockStatus = (item: Consignor) => {
    ...
    if (item.stock_status === 'selling') return <span className="inventory-status available">Sedang dijual</span>
    ...
    return <span className="inventory-status available">Sedang dijual</span>
  }
