...
export function ConsignorsPage() {
  ...
  const requestRemoval = (item: Consignor) => {
    if (!item.can_delete) {
      feedback.toast('error', 'Consignor tidak dapat dihapus', 'Data yang sudah memiliki produk, penerimaan barang, atau payout harus dipertahankan sebagai riwayat.')
      return
    }

    feedback.confirm({ title: `Hapus ${item.name}?`, description: 'Consignor ini belum memiliki transaksi atau riwayat terkait. Penghapusan bersifat permanen.', confirmLabel: 'Ya, hapus permanen', tone: 'danger', action: () => remove.mutateAsync(item.id) })
  }
