...
function ItemTable({ items }: { items: PayoutItem[] }) {
  return <div className="settlement-items"><table><thead><tr><th>Product</th><th>Customer</th><th>Sold at</th><th>Consignor price</th><th>Sale price</th><th>Profit</th></tr></thead><tbody>{items.map(item => <tr key={item.sale_id}><td><div className="settlement-product">{item.image ? <img src={item.image} alt={item.name} /> : <span><ImageIcon size={15} /></span>}<div><strong>{item.name}</strong><small>{item.code} · {item.category ?? 'No category'} · {item.brand ?? 'No brand'}</small></div></div></td><td><strong>{item.customer_name}</strong><br /><small>{item.customer_phone || 'No phone'} · {item.payment_method || '—'}</small></td><td>{shortDate(item.sold_at)}</td><td><strong>{currency(item.consignor_price)}</strong></td><td>{currency(item.sale_price)}</td><td className="profit-cell">{currency(item.profit ?? item.sale_price - item.consignor_price)}</td></tr>)}</tbody></table></div>
}
...
export function PayoutsPage() {
  ...
  const history = useQuery({ queryKey: ['payouts', 'history', status], queryFn: async () => (await api.get<ApiCollection<Payout>>('/payouts', { params: { status } })).data })
