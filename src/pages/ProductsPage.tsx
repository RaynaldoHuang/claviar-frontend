...
import type { ApiCollection, Consignor, Customer, Order, Product } from '@/types'
...
const defaultProductForm: Form = {
  code: '', name: '', consignor_id: '',
  purchase_price: '', selling_price: '', condition: 'Excellent',
...
export function ProductsPage() {
  ...
  const saleForm = useForm<SaleForm>({ resolver: zodResolver(saleSchema), defaultValues: { name: '', description: '', purchase_price: '', sale_price: '', customer_id: '', payment_method: 'Transfer', sold_at: new Date().toISOString().slice(0, 16) } })
  ...
  const openSale = (product: Product, orderId: number) => {
    ...
    saleForm.reset({ name: '', description: '', purchase_price: '', sale_price: '', customer_id: orderCustomerId || '', payment_method: 'Transfer', sold_at: new Date().toISOString().slice(0, 16) })
  }
  ...
  const edit = (product: Product) => {
    setEditing(product)
    form.reset({ code: product.code, name: product.name ?? '', consignor_id: product.consignor?.id, purchase_price: product.purchase_price ?? 0, selling_price: product.selling_price ?? 0, condition: product.condition ?? 'Excellent', status: product.status === 'pending' ? 'reserved' : product.status, description: product.description ?? '' })
    setOpen(true)
  }
  ...
  return <main className="dashboard-content">
    ...
    <Modal open={Boolean(sellingDraft)} title={activeOrderId ? 'Complete pending item' : 'Complete item'} onClose={closeSale}>{sellingDraft && <form className="form-grid sale-completion-form" onSubmit={saleForm.handleSubmit(values => completeSale.mutate(values))}>
      ...
      <label>Initial price<Controller name="purchase_price" control={saleForm.control} render={({ field }) => <RupiahInput placeholder="Contoh: 100.000" {...field} />} />{saleForm.formState.errors.purchase_price && <small>Enter a valid initial price.</small>}</label><label>Sale price<Controller name="sale_price" control={saleForm.control} render={({ field }) => <RupiahInput placeholder="Contoh: 150.000" {...field} />} />{saleForm.formState.errors.sale_price && <small>{saleForm.formState.errors.sale_price.message}</small>}</label>
    ...
    <Modal open={open} title={editing ? 'Edit product' : 'New product'} onClose={close}><form className="form-grid" onSubmit={form.handleSubmit(values => save.mutate(values))}>
      ...
      <label>Condition<input {...form.register('condition')} /></label>
      <label>Purchase price<Controller name="purchase_price" control={form.control} render={({ field }) => <RupiahInput placeholder="Contoh: 100.000" {...field} />} /></label><label>Selling price<Controller name="selling_price" control={form.control} render={({ field }) => <RupiahInput placeholder="Contoh: 150.000" {...field} />} />{form.formState.errors.selling_price && <small>{form.formState.errors.selling_price.message}</small>}</label>
