...
export function LoginPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<LoginInput>({ resolver: zodResolver(schema), defaultValues: { email: '', password: '' } })
  ...
  return <main className="login-page"><section className="login-card"><div className="login-brand"><div className="brand-mark"><Box size={21} /></div><div><strong>Claviar</strong><span>Preloved Management System</span></div></div><div className="login-copy"><p>INTERNAL ACCESS</p><h1>Welcome back</h1><span>Sign in to manage your preloved business.</span></div><form className="form-grid single" onSubmit={handleSubmit(submit)}><label>Email address<input type="email" placeholder="nama@domain.com" autoComplete="email" {...register('email')} />{errors.email && <small>{errors.email.message}</small>}</label><label>Password<input type="password" placeholder="Masukkan password" autoComplete="current-password" {...register('password')} />{errors.password && <small>{errors.password.message}</small>}</label>{error && <p className="form-error">{error}</p>}<button className="primary-button submit-button" disabled={isSubmitting}>{isSubmitting && <LoaderCircle className="spin" size={16} />}Sign in</button></form></section></main>
}
